...
// คิวรีเพื่อดึงข้อมูลรายรับรายวัน
$queryDailyRevenue = $condb->prepare("
    SELECT 
...
");
$queryDailyRevenue->execute();
$dailyRevenues = $queryDailyRevenue->fetchAll(PDO::FETCH_ASSOC);

// คิวรีเพื่อดึงข้อมูลกำไรรายวัน
$queryDailyProfit = $condb->prepare("
    SELECT 
        DATE(date_out) AS order_date,
    ...
    ORDER BY order_date ASC
");
$queryDailyProfit->execute();
$dailyProfits = $queryDailyProfit->fetchAll(PDO::FETCH_ASSOC);
...
$dailyDates = array_column($dailyRevenues, 'order_date');
$dailyRevenueValues = array_column($dailyRevenues, 'revenue');
$dailyProfitValues = array_column($dailyProfits, 'profit');
...
<head>
    ...
    <title>Dashboard รายรับและกำไร</title>
            ...
            data: {
                ...
                datasets: [
                    ...
                    },
                    {
                        ...
                        data: <?= json_encode($dailyProfitValues); ?>,
